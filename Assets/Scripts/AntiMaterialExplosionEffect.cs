...
/// <summary>
/// Short-lived anti-material detonation fireball.
/// </summary>
public class AntiMaterialExplosionEffect : MonoBehaviour
{
    public const float FireDurationSeconds = 1f;
    public const float FireDiameterMeters = 8f;
    public static float FireRadiusMeters => FireDiameterMeters * 0.5f;

    const float DurationSeconds = FireDurationSeconds;
    const float DiameterMeters = FireDiameterMeters;
    const float ExpandSeconds = 0.14f;

    static readonly List<AntiMaterialExplosionEffect> LiveEffects = new List<AntiMaterialExplosionEffect>();

    float _elapsed;
    Material _outerMaterial;
    Material _midMaterial;
    Material _coreMaterial;
    Renderer _outerRenderer;
    ...
    {
        var effectObject = new GameObject("Anti-Material Explosion");
        effectObject.transform.position = center;
        var effect = effectObject.AddComponent<AntiMaterialExplosionEffect>();
    ...
    void BuildVisuals()
    {
        _outer = CreateFireLayer("Outer Fire", new Color(1f, 0.42f, 0.08f, 1f), out _outerRenderer, out _outerMaterial);
        _mid = CreateFireLayer("Mid Fire", new Color(1f, 0.72f, 0.16f, 1f), out _midRenderer, out _midMaterial);
        _core = CreateFireLayer("Core Fire", new Color(1f, 0.95f, 0.72f, 1f), out _coreRenderer, out _coreMaterial);
    }

    Transform CreateFireLayer(string name, Color color, out Renderer renderer, out Material material)
    {
    ...
    static Material CreateFireMaterial(Color color)
    {
        ...
        {
            name = "Anti-Material Explosion Fire",
            color = color
        };
